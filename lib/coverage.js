...
function collectPackageEntrypoints(cwd, entrypoints, options = {}) {
  ...
  for (const [name, command] of Object.entries(scripts)) {
    ...
    const guardedByScript = isGuarded(command, scripts) || isGuarded(prehook, scripts);
    entrypoints.push({
      type: 'package-script',
      file: 'package.json',
      name,
      command,
      guarded: guardedByScript || Boolean(options.npmGuardActive),
      guard: guardedByScript ? 'execfence-run' : (options.npmGuardActive ? 'npm-guard' : null),
    });
  }

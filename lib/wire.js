...
function wirePackageJson(cwd, changes, options) {
  ...
  let changed = false;
  for (const name of ['test', 'build', 'dev', 'start', 'pack', 'publish']) {
    if (scripts[name] && shouldWrap(scripts[name])) {
...
function wireWorkflows(cwd, changes, options) {
  ...
  for (const name of fs.readdirSync(dir).filter((file) => /\.ya?ml$/i.test(file))) {
    ...
    const next = original.replace(/^(\s*-\s*run:\s*)(npm\s+(?:test|run\s+build|run\s+dev|pack|publish)\b.*)$/gm, (full, prefix, command) => {
...
function shouldWrap(command = '') {
  ...
  return !/execfence\s+run|execfence:ci|execfence\s+ci/.test(text) &&
    /\b(?:npm\s+(?:test|run\s+build|run\s+dev|pack|publish)|node\b|go\s+test|cargo\s+test|python\s+-m\s+pytest|pytest\b|vite\b|next\b|webpack\b|tauri\b)/.test(text);
}

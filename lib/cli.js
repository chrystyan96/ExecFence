...
const { comparePackageVersions, formatReviewText, reviewDependenciesConcurrent } = require('./deps-review');
...
const { explainPolicy, learnPolicyFromReport, testPolicy } = require('./policy');
const { diffPolicy } = require('./policy-diff');
const { generateSbom, writeSbom } = require('./sbom');
const {
  addApprover,
  auditApprovals,
  createApproval,
...
const { githubAnnotations, writeCiArtifacts } = require('./ci-adapters');
const {
  explainSandbox,
  helperAudit,
  initSandbox,
...
const commandGroups = [
  {
    ...
    commands: [
      ['execfence run [--sandbox] [--sandbox-mode audit|enforce] [--allow-degraded --degraded-reason <reason>] [--record-artifacts] [--deny-on-new-executable] [--dependency-behavior-audit] [--timeout-ms <ms>] -- <command>', 'Scan before running a command; --sandbox enforce requires a verified helper. Degraded execution requires an explicit reason and is forbidden in CI.'],
  ...
  },
  {
    ...
    commands: [
      ...
      ['execfence explain <finding-id> [--report <file>] [--format text|json]', 'Explain the exact evidence, confidence, decision, policy reason, and remediation.'],
    ],
  },
  {
    ...
    commands: [
      ...
      ['execfence guard disable [--apply]', 'Preview or remove generated project wrappers/rules while preserving evidence.'],
  ...
  },
  {
    title: 'CI/release',
    commands: [
      ['execfence ci [--base-ref <ref>] [--format json|sarif] [--sarif <file>] [--github-annotations]', 'Run the CI guardrail bundle and optionally emit SARIF/GitHub annotations.'],
  ...
  },
  {
    ...
    commands: [
      ...
      ['execfence deps review [--base-ref <ref>] [--format text|json] [--concurrency <n>] [--timeout-ms <ms>] [--package-manager auto|npm|pnpm|yarn|bun|pip|uv|poetry|cargo|go|maven|gradle|nuget|composer|bundler]', 'Review changed packages across supported ecosystems with bounded concurrent metadata checks.'],
      ['execfence deps compare <name@old> <name@new> [--package-manager <manager>]', 'Compare scripts, binaries, provenance, integrity, and archive contents between two package versions.'],
      ['execfence sbom [--format cyclonedx|spdx] [--output <file>]', 'Generate a monorepo-aware software bill of materials.'],
      ['execfence manifest', 'Print the current execution-surface manifest without changing its approved baseline.'],
      ['execfence manifest diff', 'Detect new or changed execution entrypoints without changing the approved baseline.'],
      ['execfence manifest approve', 'Explicitly approve and write the current execution-surface manifest.'],
  ...
  },
  {
    ...
    commands: [
      ...
      ['execfence policy explain|test|diff|learn [--report <file>]', 'Explain, validate, compare, or propose non-mutating policy approvals from evidence.'],
      ['execfence trust add <target> --type file|registry|action|package-scope|package-source --reason <reason> --owner <owner> --expires-at <date>', 'Trust a reviewed artifact, registry, action, scope, or package source.'],
      ['execfence trust audit', 'Audit configured trust stores for drift and expiry.'],
      ['execfence approval create|sign|audit|list|key-add', 'Create scoped approvals, add trusted approver keys, sign approvals, and verify quorum/expiry.'],
    ],
  },
  {
    title: 'Sandbox',
...
async function main(args) {
  ...
  if (command === 'run') {
    ...
    const optionArgs = delimiter >= 0 ? args.slice(1, delimiter) : [];
    const result = runWithFence(commandArgs, {
      sandbox: optionArgs.includes('--sandbox'),
      sandboxMode: readOption(optionArgs, '--sandbox-mode'),
      allowDegraded: optionArgs.includes('--allow-degraded') && Boolean(readOption(optionArgs, '--degraded-reason')),
      degradedReason: readOption(optionArgs, '--degraded-reason'),
      recordArtifacts: optionArgs.includes('--record-artifacts'),
      denyOnNewExecutable: optionArgs.includes('--deny-on-new-executable'),
      dependencyBehaviorAudit: optionArgs.includes('--dependency-behavior-audit'),
      timeoutMs: readOption(optionArgs, '--timeout-ms'),
    });
  ...
  if (command === 'ci') {
    ...
    writeAutomaticReport(result, { command: `execfence ${args.join(' ')}` });
    const artifacts = writeCiArtifacts(process.cwd(), result, {
      sarif: readOption(args.slice(1), '--sarif'),
      summary: readOption(args.slice(1), '--summary'),
    });
    if (args.includes('--github-annotations')) {
      for (const annotation of githubAnnotations(result)) console.log(annotation);
    }
    const format = readOption(args.slice(1), '--format') || 'json';
    console.log(format === 'sarif' ? formatResult(result, 'sarif') : JSON.stringify({ ...result, artifacts }, null, 2));
    if (!result.ok) {
      process.exitCode = 1;
    }
    return;
  ...
  if (command === 'deps') {
    await depsCommand(args.slice(1));
    return;
  }
  if (command === 'manifest') {
    if (args[1] === 'diff') {
    ...
    if (args[1] === 'approve' || args[1] === 'update') {
      const current = generateManifest(process.cwd());
      const config = require('./config').loadConfig(process.cwd()).config;
      if (config.approvals?.requireSignedManifest) {
        const { activeApprovalFor } = require('./approvals');
        const approval = activeApprovalFor(process.cwd(), 'manifest', { manifestHash: current.contentHash }, {
          cwd: process.cwd(),
          environment: process.env.CI ? 'ci' : 'local',
          command: 'execfence manifest approve',
        });
        if (!approval) {
          throw new Error(`Manifest ${current.contentHash} requires a valid signed approval before it can be written.`);
        }
      }
      const manifest = writeManifest(process.cwd(), current);
      console.log(JSON.stringify({ ...manifest, approved: true }, null, 2));
      return;
    }
    const manifest = generateManifest(process.cwd());
    console.log(JSON.stringify({ manifest, approved: false, note: 'Run execfence manifest approve after review to update the baseline.' }, null, 2));
    return;
  }
  if (command === 'sbom') {
    const format = readOption(args.slice(1), '--format') || 'cyclonedx';
    const output = readOption(args.slice(1), '--output');
    const result = output
      ? writeSbom(process.cwd(), { format, output, packageManager: readOption(args.slice(1), '--package-manager') })
      : { document: generateSbom(process.cwd(), { format, packageManager: readOption(args.slice(1), '--package-manager') }) };
    console.log(JSON.stringify(result, null, 2));
    return;
  }
  if (command === 'report') {
  ...
  if (command === 'explain') {
    console.log(explainFinding(args[1], {
      cwd: process.cwd(),
      reportPath: readOption(args.slice(1), '--report'),
      format: readOption(args.slice(1), '--format') || 'text',
    }));
    return;
  }
  if (command === 'detect') {
...
async function depsCommand(args) {
  ...
  if (subcommand === 'diff') {
    ...
    result.review = await reviewDependenciesConcurrent(process.cwd(), {
      baseRef: readOption(args, '--base-ref') || 'HEAD',
      packageManager: readOption(args, '--package-manager') || 'auto',
      concurrency: readOption(args, '--concurrency'),
      operationTimeoutMs: readOption(args, '--timeout-ms'),
    });
  ...
  if (subcommand === 'review') {
    const result = await reviewDependenciesConcurrent(process.cwd(), {
      baseRef: readOption(args, '--base-ref') || 'HEAD',
      packageManager: readOption(args, '--package-manager') || 'auto',
      concurrency: readOption(args, '--concurrency'),
      operationTimeoutMs: readOption(args, '--timeout-ms'),
  ...
  if (subcommand === 'compare') {
    if (!args[1] || !args[2]) throw new Error('Usage: execfence deps compare <name@old> <name@new> [--package-manager <manager>]');
    const result = comparePackageVersions(process.cwd(), args[1], args[2], {
      packageManager: readOption(args, '--package-manager') || 'npm',
    });
    console.log(JSON.stringify(result, null, 2));
  ...
  throw new Error('Usage: execfence deps diff|review|compare [--base-ref <ref>] [--format text|json] [--package-manager auto|npm|pnpm|yarn|bun|pip|uv|poetry|cargo|go|maven|gradle|nuget|composer|bundler]');
}
...
function guardCommand(args) {
  ...
  if (subcommand === 'disable') {
    console.log(JSON.stringify(guardDisable(process.cwd(), { apply: args.includes('--apply') }), null, 2));
    return;
  }
...
function policyCommand(args) {
  ...
  if (subcommand === 'diff') {
    const result = diffPolicy(process.cwd(), { baseRef: readOption(args, '--base-ref') || 'HEAD' });
    console.log(JSON.stringify(result, null, 2));
    if (!result.ok) process.exitCode = 1;
    return;
  }
  if (subcommand === 'learn') {
    console.log(JSON.stringify(learnPolicyFromReport(process.cwd(), readOption(args, '--report')), null, 2));
    return;
  }
  throw new Error('Usage: execfence policy explain|test|diff|learn [--policy-pack <name>] [--base-ref <ref>] [--report <file>]');
}
...
function sandboxCommand(args) {
  ...
  if (subcommand === 'plan') {
    ...
    const result = sandboxPlan(process.cwd(), commandArgs, {
      mode: readOption(optionArgs, '--sandbox-mode') || readOption(optionArgs, '--mode') || 'audit',
      profile: readOption(optionArgs, '--profile'),
      allowDegraded: optionArgs.includes('--allow-degraded') && Boolean(readOption(optionArgs, '--degraded-reason')),
      degradedReason: readOption(optionArgs, '--degraded-reason'),
    });
    console.log(JSON.stringify(result, null, 2));
...
function trustCommand(args) {
  ...
  throw new Error('Usage: execfence trust add <target> --type file|registry|action|package-scope|package-source --reason <reason> --owner <owner> --expires-at <date>\n       execfence trust audit');
}
...
function approvalCommand(args) {
  ...
  if (subcommand === 'create') {
    const type = readOption(args, '--type') || 'finding';
    const result = createApproval(process.cwd(), {
      type,
      subject: approvalSubject(args, type),
      scope: {
        branch: readOption(args, '--branch'),
        command: readOption(args, '--command-scope'),
      ...
      owner: readOption(args, '--owner'),
      expiresAt: readOption(args, '--expires-at'),
      requiredSignatures: readOption(args, '--required-signatures'),
    });
    console.log(JSON.stringify(result, null, 2));
    return;
  }
  if (subcommand === 'key-add') {
    const result = addApprover(process.cwd(), {
      keyId: readOption(args, '--key-id'),
      publicKey: readOption(args, '--public-key'),
      owner: readOption(args, '--owner'),
      reason: readOption(args, '--reason'),
      expiresAt: readOption(args, '--expires-at'),
    });
  ...
  if (subcommand === 'sign') {
    const result = signApproval(process.cwd(), args[1], {
      keyId: readOption(args, '--key-id'),
      privateKey: readOption(args, '--private-key'),
      passphrase: process.env.EXECFENCE_KEY_PASSPHRASE,
    });
    console.log(JSON.stringify(result, null, 2));
    return;
  }
  if (subcommand === 'audit' || subcommand === 'list') {
    const result = auditApprovals(process.cwd());
    console.log(JSON.stringify(subcommand === 'list' ? result.approvals : result, null, 2));
    if (subcommand === 'audit' && !result.ok) process.exitCode = 1;
    return;
  }
  throw new Error('Usage: execfence approval create --type finding|manifest|manifest-entry|package|command|policy --finding-id <id>|--manifest-hash <sha256>|--package <name@version>|--command <command>|--policy-hash <sha256> --reason <reason> --owner <owner> --expires-at <date-time> [--required-signatures <n>] [--branch <name>] [--command-scope <regex>] [--environment local|ci]\n       execfence approval key-add --key-id <id> --public-key <file> --owner <owner> --reason <reason> --expires-at <date-time>\n       execfence approval sign <id> --key-id <id> --private-key <file>\n       execfence approval audit|list');
}

function approvalSubject(args, type) {
  if (type === 'finding') {
    return compactObject({
      findingId: readOption(args, '--finding-id'),
      file: readOption(args, '--file'),
      sha256: readOption(args, '--sha256'),
    });
  }
  if (type === 'manifest' || type === 'manifest-entry') {
    return compactObject({
      manifestHash: readOption(args, '--manifest-hash'),
      entrypointId: readOption(args, '--entrypoint-id'),
      file: readOption(args, '--file'),
      sha256: readOption(args, '--sha256'),
    });
  }
  if (type === 'package') {
    return compactObject({ package: readOption(args, '--package'), integrity: readOption(args, '--integrity') });
  }
  if (type === 'command') {
    return compactObject({ command: readOption(args, '--command'), sha256: readOption(args, '--sha256') });
  }
  return compactObject({ policyHash: readOption(args, '--policy-hash'), file: readOption(args, '--file') });
}

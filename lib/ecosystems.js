...
const { parseToolCommand } = require('./command-parser');
...
const adapters = [
  {
    ...
    safe: ['audit', 'cache', 'config', 'doctor', 'fund', 'help', 'login', 'logout', 'outdated', 'owner', 'ping', 'profile', 'search', 'team', 'token', 'view', 'whoami'],
...
function classifyCommand(tool, args = []) {
  ...
  const parsed = commandForTool(normalizedTool, args, adapter);
  const command = parsed.command;
  const suspiciousToken = args.some((arg) => adapter.risky.includes(String(arg)) || adapter.installLike.includes(String(arg)) || adapter.runtimeLike.includes(String(arg)));
  return {
    risky: suspiciousToken || parsed.ambiguous || adapter.risky.includes(command) || adapter.installLike.includes(command) || adapter.runtimeLike.includes(command),
...
function commandForTool(tool, args = [], adapter = adapterForTool(tool)) {
  if (tool === 'python' || tool === 'python3') {
    if (args[0] === '-m') return { command: args[1] || '-m', commandIndex: 1, ambiguous: false };
    if (args[0] === 'setup.py') return { command: 'setup.py', commandIndex: 0, ambiguous: false };
  }
  if (tool === 'uv' && args[0] === 'pip') {
    const nested = parseToolCommand('pip', args.slice(1), adapter?.risky || []);
    return { ...nested, commandIndex: nested.commandIndex < 0 ? 0 : nested.commandIndex + 1 };
  }
  return parseToolCommand(tool, args, Array.from(new Set([
    ...(adapter?.risky || []),
    ...(adapter?.installLike || []),
    ...(adapter?.runtimeLike || []),
    ...(adapter?.safe || []),

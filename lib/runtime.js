...
function runWithFence(commandArgs, options = {}) {
  ...
  const child = sandboxEnforce
    ? runSandboxedCommand(cwd, commandArgs, sandbox.plan, { stdio: options.stdio || 'inherit' })
  ...
  return {
    ...
    baselinePath: preflight.baselinePath,
    roots: preflight.roots,
    findings: [...(preflight.findings || []), ...(postflight.findings || []), ...artifactFindings, ...helperFindings, ...behaviorFindings],
    ...
    warningFindings: [...(preflight.warningFindings || []), ...(postflight.warningFindings || []), ...behaviorFindings.filter((finding) => !['critical', 'high'].includes(finding.severity))],
...
function sandboxHelperFindings(cwd, child, sandbox) {
  ...
  if (child.status && child.status !== 0 && findings.length === 0) {
    findings.push({
      id: 'sandbox-helper-command-failed',
      severity: 'high',
      file: path.relative(cwd, sandbox.plan.configPath || sandboxFileFallback()).replaceAll(path.sep, '/') || 'sandbox helper',
      line: 1,
      detail: `Sandbox helper supervised command exited with status ${child.status}.`,
      threatCategory: 'containment-gap',
      activationSurface: 'run',
    });
  }

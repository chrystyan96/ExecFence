...
const shimTools = ['npm', 'npx', 'pnpm', 'yarn', 'yarnpkg'];
...
function npmGuardStatus(options = {}) {
  ...
  const activeInPath = pathEntries.some((entry) => samePath(entry, dir));
  const real = {
  ...
  return {
    ...
    note: !enabled
      ...
      : activeInPath
      ? 'npm/npx/pnpm/yarn resolve through ExecFence shims in the current PATH.'
...
function globalGuardCoverageGaps(status) {
  ...
  if (status.real.bun) {
    gaps.push('bun is installed but not intercepted by global guard shims');
  }
  gaps.push('CI, containers, IDE-integrated package managers, and package-manager wrappers still need project/CI guardrails');
  return gaps;
}

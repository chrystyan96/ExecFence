...
const { optionTakesValue } = require('./command-parser');
...
function npmGuardStatus(options = {}) {
  ...
  return {
    enabled,
    coverageActive: Boolean(enabled && activeInPath),
    activeInPath,
    presentInPath,
    shimDir: dir,
    real,
    delegation: 'resolved absolute package-manager path outside the shim directory',
    shims,
    tools,
    profiles,
  ...
  if (classification.risky) {
    ...
    if (classification.installLike) {
      ...
      if (specs.length > 0 || options.reviewInstallWithoutSpecs) {
        ...
        if (!review.ok) {
          const reviewReport = writeReport({
            cwd,
          ...
          console.error(`[execfence] package metadata review blocked before ${normalizedTool} started. Report: ${reviewReport.filePath}`);
          return { ok: false, blocked: true, exitCode: 1, tool: normalizedTool, args, report: reviewReport.filePath, classification, review };
        }
      }
...
function packageSpecsFromArgs(tool, args = []) {
  ...
  const command = fullClassification.command;
  const commandIndex = Number.isInteger(fullClassification.commandIndex) ? fullClassification.commandIndex : args.indexOf(command);
  const afterCommand = commandIndex >= 0 ? args.slice(commandIndex + 1) : [];
  if (tool === 'npx') {
    return positionalArguments(tool, args).slice(0, 1);
  }
  if (!fullClassification.installLike) {
    return [];
  }
  if (tool === 'go') {
    if (command === 'get') return afterCommand.filter((arg) => arg && !arg.startsWith('-'));
    if (command === 'install') return afterCommand.filter((arg) => /@/.test(arg));
    return [];
  }
  if (tool === 'cargo') {
    if (command === 'add') return afterCommand.filter((arg) => arg && !arg.startsWith('-'));
    if (command === 'install') return afterCommand.filter((arg) => arg && !arg.startsWith('-'));
    return [];
  }
  if (tool === 'dotnet') {
  ...
  return positionalArguments(tool, afterCommand);
}

function positionalArguments(tool, args) {
  const values = [];
  for (let index = 0; index < args.length; index += 1) {
    const arg = args[index];
    if (!arg || arg === '--') continue;
    if (arg.startsWith('-')) {
      if (!arg.includes('=') && (optionTakesValue(tool, arg) || arg === '--save-prefix')) index += 1;
      continue;
    }
    values.push(arg);
  }
...
function detectYarnMajor(real, env = process.env) {
  ...
  try {
    const child = spawnSync(yarn, ['--version'], { encoding: 'utf8', stdio: ['ignore', 'pipe', 'ignore'], env });

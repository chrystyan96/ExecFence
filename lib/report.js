...
const { reportsDir: defaultReportsDir, resolveProjectPath } = require('./paths');
...
function writeReport(result, options = {}) {
  ...
  const reportDir = options.reportDir
    ? path.resolve(cwd, options.reportDir)
    : resolveProjectPath(cwd, result.config?.reportsDir, defaultReportsDir);
...
function enrichFinding(cwd, finding) {
  ...
  const findingSnippet = exists ? snippet(filePath, finding.line || 1) : '';
  return {
    ...finding,
    column: finding.column || null,
    symbol: exists ? inferSymbol(filePath, finding.line || 1) : null,
    ...
    confidence: finding.confidence || confidenceFor(finding.id),
...
function buildBlockingSummary(findings, result = {}) {
  ...
  const blocking = findings.filter((finding) => (
    finding.decision === 'block' || (!finding.decision && blockingSeverities.has(finding.severity || finding.rule?.severity || 'high'))
  ));

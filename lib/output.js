...
function formatFindings(findings, result = {}) {
  ...
  return [
    ...
    ...blocked.map((item) => `- [${item.decision || 'block'}/${item.severity || 'high'}/${item.confidence || 'medium'} confidence] ${item.id}: ${item.file}:${item.line || 1} - ${item.detail}`),
    ...warnings.map((item) => `- [${item.decision || 'review'}/${item.severity || 'medium'}/${item.confidence || 'medium'} confidence] ${item.id}: ${item.file}:${item.line || 1} - ${item.detail}`),
  ].join('\n');
}

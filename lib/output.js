...
function formatFindings(findings, result = {}) {
  ...
  const warnings = findings.filter((item) => !blockedKeys.has(`${item.id}:${item.file}:${item.line || 1}`));
  return [
    result.mode === 'audit'
      ? '[security-guardrails] Audit findings:'
      : blocked.length > 0 ? '[security-guardrails] Suspicious artifact(s) blocked:' : '[security-guardrails] Suspicious artifact(s) found:',
    ...
    runs: [
      {
        tool: {
          driver: {
            ...
            informationUri: 'https://github.com/chrystyan96/security-guardrails',

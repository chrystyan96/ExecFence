...
function sandboxCapabilities(cwd = process.cwd(), options = {}) {
  ...
  const helperCaps = helper.metadata?.capabilities || {};
  const filesystem = hasHelper && helperCaps.filesystem && helperCaps.sensitiveReads && helperCaps.newExecutables ? 'yes' : 'degraded';
  const processSupervision = hasHelper && helperCaps.process && helperCaps.childProcesses ? 'yes' : 'degraded';
  const network = hasHelper && helperCaps.network ? 'yes' : 'no';
  ...
  return {
    ...
    filesystem: {
      ...
      detail: filesystem === 'yes' ? 'Verified helper can block sensitive reads and new executable/archive writes.' : 'Built-in mode can snapshot and rescan files, but cannot block writes or sensitive reads before they happen.',
    },
    process: {
      supervision: processSupervision,
      detail: processSupervision === 'yes' ? 'Verified helper can supervise and block child processes.' : 'Built-in mode records the root process only and cannot block child processes.',
    },
    network: {
      enforcement: network,
      detail: network === 'yes' ? 'Verified helper can block outbound network outside allowlists.' : 'Built-in mode cannot block outbound network connections.',
    },
    helper,
    missingForEnforce,
  };
...
function helperAudit(cwd = process.cwd(), options = {}) {
  ...
  const capabilities = normalizeHelperCapabilities(metadata.capabilities);
  for (const capability of requiredHelperCapabilities) {
    if (!capabilities[capability]) {
      issues.push(`missing helper capability: ${capability}`);
    }
  }
...
function normalizeHelperCapabilities(value = {}) {
  return Object.fromEntries(requiredHelperCapabilities.map((capability) => [capability, Boolean(value[capability])]));
}

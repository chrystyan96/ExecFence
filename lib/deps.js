...
function depsDiff(cwd = process.cwd(), options = {}) {
  ...
  const currentFiles = dependencyFiles(cwd);
  const previousFiles = dependencyFiles(cwd, { ref: baseRef });
  const files = Array.from(new Set([...currentFiles, ...previousFiles])).sort();
  const current = collectDependencies(cwd, { files, packageManager: options.packageManager || 'auto' });
  const previous = collectDependencies(cwd, { ref: baseRef, files, packageManager: options.packageManager || 'auto' });
  const configuredChecks = loadConfig(cwd).config?.deps || {};
  const checks = {
    ...configuredChecks,
    ...(options.config || {}),
  ...
  for (const [key, dep] of afterMap.entries()) {
    ...
    if (!before) {
      added.push(dep);
      findings.push(...findingsForNewDependency(dep, current.dependencies, checks));
      continue;
    }
    if (normalizeSource(before.source) !== normalizeSource(dep.source) || before.version !== dep.version || before.registry !== dep.registry) {
      ...
      changed.push(item);
      findings.push(...findingsForChangedDependency(item, checks));
    }
  }
  ...
  try {
    ...
    return Object.entries(parsed.packages || {})
      ...
      .map(([name, value]) => dependency({
        ecosystem: 'npm',
        name: packageNameFromInstallPath(name),
...
function goDependency(name, version, lockfile, replaceTargets = new Map()) {
  ...
  return dependency({
    ...
    version,
    source,
    registry: replace ? (registryFor(replace) || 'path') : 'proxy.golang.org',
    lockfile,
    metadata: {
      replace: replace || '',
...
function parseGoSum(content, lockfile) {
  ...
  for (const line of content.split(/\r?\n/)) {
    ...
    if (match && !match[2].endsWith('/go.mod')) {
      deps.push(dependency({ ecosystem: 'go', name: match[1], version: match[2], source: match[1], registry: 'proxy.golang.org', lockfile }));
    }
  }
  return deps;
}
...
function findingsForNewDependency(dep, allDeps, checks = {}) {
  const findings = [];
  if (checks.detectSuspiciousSources !== false && isSuspiciousSource(dep.source)) {
...
function findingsForChangedDependency(change, checks = {}) {
  ...
  if (checks.detectRegistryDrift !== false && change.before.registry !== change.after.registry && change.after.registry) {
  ...
  if (checks.detectSuspiciousSources !== false && isSuspiciousSource(change.after.source || '')) {
...
function depKey(dep) {
  const slot = dep.metadata?.slot || dep.name;
  return `${dep.ecosystem}:${dep.lockfile || '<explicit>'}:${slot}`;
}

function dedupeDependencies(dependencies) {
  const map = new Map();
  for (const dep of dependencies) {
    map.set(depKey(dep), dep);
  }
...
function dependencyFiles(cwd, options = {}) {
  if (options.ref) {
    try {
      return execFileSync('git', ['ls-tree', '-r', '--name-only', options.ref], {
        cwd,
        encoding: 'utf8',
        stdio: ['ignore', 'pipe', 'ignore'],
      }).split(/\r?\n/)
        .map((file) => file.trim().replaceAll('\\', '/'))
        .filter((file) => file && !isIgnoredDependencyPath(file) && isDependencyFile(file))
        .sort();
    } catch {
      return [];
    }
  ...
  const files = [];
  walkDependencyFiles(path.resolve(cwd), path.resolve(cwd), files);
  return files.sort();
}

function walkDependencyFiles(root, current, files) {
  let entries;
  try {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      if (!isIgnoredDependencyPath(relative)) {
        walkDependencyFiles(root, fullPath, files);
      }
    } else if (entry.isFile() && isDependencyFile(relative)) {
      files.push(relative);
    }
  }
}
...
function isDependencyFile(file) {
  ...
  return rootDependencyFiles.includes(base) || /^requirements.*\.txt$/i.test(base) || /\.csproj$/i.test(base);
}

function isIgnoredDependencyPath(file) {
  return String(file).split('/').some((part) => new Set([
    '.git', '.execfence', 'node_modules', 'vendor', 'dist', 'build', 'target',
    '.next', '.turbo', '.venv', 'venv', '__pycache__',
  ]).has(part));
}

function packageNameFromInstallPath(installPath) {
  const normalized = String(installPath).replaceAll('\\', '/');
  const marker = '/node_modules/';
  const index = normalized.lastIndexOf(marker);
  if (index >= 0) {
    return normalized.slice(index + marker.length);
  }
  return normalized.replace(/^node_modules\//, '');
}

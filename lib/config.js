...
function loadConfig(cwd = process.cwd(), explicitPath) {
  ...
  const signaturesPath = config.signaturesFile
    ? resolveProjectPath(cwd, config.signaturesFile)
...
function createDefaultConfig(cwd = process.cwd(), options = {}) {
  ...
  const config = {
    ...
    ci: {
      enabled: true,
      baseRef: 'HEAD',
      checks: ['scan', 'manifest-diff', 'deps-diff', 'coverage', 'config-validate', 'pack-audit', 'trust-audit', 'policy-diff', 'approval-audit'],
    },
    wire: {
      packageScripts: true,
      workflows: true,
    ...
    approvals: {
      path: '.execfence/approvals.json',
      approversPath: '.execfence/trust/approvers.json',
      requireSignedFindings: true,
      requireSignedManifest: false,
      requireSignedPolicyChanges: false,
      minimumSignatures: 1,
    },
    trustStore: {

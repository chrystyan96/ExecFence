...
function runCi(cwd = process.cwd(), options = {}) {
  ...
  return {
    cwd: root,
    ok: scanResult.ok && manifestDiff.ok && deps.ok && depsReview.ok && pack.ok && trust.ok && blockedFindings.length === 0,
    mode: 'ci',
...
function findingsFromCoverage(coverage, supplyChainConfig = {}) {
  ...
  return (coverage.uncovered || [])
    .filter((entry) => entry.type === 'package-manager-surface' || /\b(?:npm|pnpm|yarn|bun)\b/.test(entry.command || ''))
    .map((entry) => ({
  ...
  for (const item of diff.risk || []) {
    ...
    findings.push({
      ...
      remediation: entry.guarded
        ? 'Review the new or changed guarded entrypoint and update the manifest baseline after approval.'

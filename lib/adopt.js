...
const { baselineFileName, resolveProjectPath } = require('./paths');
...
function adoptProject(cwd = process.cwd(), options = {}) {
  ...
  const blockDuringAdoption = Boolean(options.blockDuringAdoption || scanResult.config?.adopt?.blockDuringAdoption);
  const decision = evaluateFindings(scanResult.findings || [], {
    mode: blockDuringAdoption ? 'block' : 'audit',
  ...
  if (options.writeSuggestedBaseline || scanResult.config?.adopt?.writeSuggestedBaseline) {
...
function suggestedBaselineFor(cwd, findings) {
  return findings.map((finding) => {
    ...
    return {
      ...
      file: finding.file,
      sha256: filePath && fs.existsSync(filePath) && fs.statSync(filePath).isFile() ? sha256File(filePath) : null,

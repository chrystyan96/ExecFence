...
function enrichFindings(cwd, findings = [], config = {}) {
  const enrichmentConfig = config.analysis?.webEnrichment || {};
  if (enrichmentConfig.enabled === false) {
    return { status: 'skipped', reason: 'analysis.webEnrichment.enabled is false', sources: [], results: [], errors: [] };
  }
  const automatic = new Set(enrichmentConfig.automaticForSeverities || ['critical', 'high']);
  const blocked = findings.filter((finding) => automatic.has(finding.severity || 'high'));
  if (blocked.length === 0) {
  ...
  const ttlMs = Number(config.enrichment?.cacheTtlMs || enrichmentConfig.cacheTtlMs || 24 * 60 * 60 * 1000);
  const results = [];
  const errors = [];
